...
#            btn.Refresh()
    ...
    def OnAboutDlg(self, event):
        ...
        info.Version = "0.0.6 Beta"
        info.Copyright = """(C) 2014-2016 Mustafa Tekpinar\n
        Contributors: Omer Faruk Ozdemir\n
...
#####################
    ...
    def OnIonize(self, event):
        ...
        negIon=""
        ion_concentration=""
        ions_mdp_lt_5="""; ions.mdp - used as input into grompp to generate ions.tpr
...
"""
        ...
        ions_mdp_gt_5="""; ions.mdp - used as input into grompp to generate ions.tpr
; Parameters describing what to do, when to stop and what to save
integrator     = steep     ; Algorithm (steep = steepest descent minimization)
emtol          = 1000.0    ; Stop minimization when the maximum force < 1000.0 kJ/mol/nm
emstep         = 0.01      ; Energy step size
...
"""
        ...
        if (dlg.ShowModal()==wx.ID_OK):
            ...
            if(ion_concentration !=""):
                ionize_command=ionize_command+" -conc "+ion_concentration    

                error_message="ERROR: Something went wrong in ionization procedure! Check log files!"
                if(version<5.0):
                    ions_mdp_file=open(dir+"/ions.mdp", 'w')
                    ions_mdp_file.write(ions_mdp_lt_5)
                    ions_mdp_file.close()
                    status=os.system("grompp -f "+dir+"/ions.mdp "+preprocess_command)
                    if(status==0):
                #Produce and place ions
                    ...
                    ions_mdp_file=open(dir+"/ions.mdp", 'w')
                    ions_mdp_file.write(ions_mdp_gt_5)
                    ions_mdp_file.close()
                    status=os.system("gmx grompp -f "+dir+"/ions.mdp "+preprocess_command)
                    if(status==0):
                #Produce and place ions
...
#                self.drawPotential(event)
    ...
    def OnEquilibratePhase1(self, event):
        ...
        if(status!=(-1)):
            ...
            if(status1 == 0):
                status2=os.system("gmx mdrun -v -deffnm "+dir+"/"+"nvt")
                if(status2 == 0):
    ...
    def OnEquilibratePhase2(self, event):
        ...
        if(status!=(-1)):
            ...
            if(status1 == 0):
                status2=os.system("gmx mdrun -v -deffnm "+dir+"/"+"npt")
                if(status2 == 0):
