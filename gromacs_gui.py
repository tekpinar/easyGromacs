...
#                print line
 ...
 #       self.x_max=1000.0
    ...
    def drawTemperaturevsTime(self, event):
        data_file=open("temperature.xvg", "r")
        lines=data_file.readlines()
        md_columns=[]
        md_pc1=[]
        md_pc2=[]
...
#        self.x_max=1000.0
    ...
    def prepare(self, event):
        ...
        if(initial_pdb != ""):
            ...
            prepare_command="pdb2gmx -f "+initial_pdb+" -o processed.pdb -water "+solvent_model.lower()+" -ff "+force_field[0:14]
            if(version<5.0):
    ...
    def solvate(self, event):
        ...
        presolvate_command="editconf -f processed.pdb -o newbox.pdb -bt "+boxtype+" -c -d 1.0"
        if(boxtype != ""):
            if(version<5.0):
    ...
    def ionize(self, event):
        ...
        preprocess_command="-c solvated.pdb -p topol.top -o ions.tpr"
    ...
    def OnMinimize(self, event):
        pre_min_command=" -c solv_ions.pdb -p topol.top -o em.tpr"
        if(version<5.0):
        ...
        if(status1 == 0):
            ...
            if(status2==0):
                status2=os.system("echo 10 0|g_energy -f em.edr -o potential.xvg")
#                    self.changePlot(event)
    ...
    def OnEquilibratePhase1(self, event):
        pre_phase1_command=" -c em.gro -p topol.top -o nvt.tpr"
        if(version<5.0):
        ...
        if(status1 == 0):
            status2=os.system("mdrun -v -deffnm nvt")
            if(status2 == 0):
                os.system("echo 15 0|gmx energy -f nvt.edr -o temperature.xvg")
...
#        self.Show(True)
    ...
    def OnEquilibratePhase2(self, event):
        pre_phase2_command=" -c nvt.gro -p topol.top -o npt.tpr"
        if(version<5.0):
        ...
        if(status1 == 0):
            status2=os.system("mdrun -v -deffnm npt")
            if(status2 == 0):
                os.system("echo 16 0|gmx energy -f npt.edr -o pressure.xvg")
